...
pub(crate) fn ask_for(message: &str) -> Option<String> {
    let username = input::<String>()
        .msg(format!(
            "{} (Enter with no input to return to previous screen): ",
            message
        ))
        .get();
...
pub(crate) fn ask_for_password() -> Option<SecretString> {
    let username = input::<SecretString>()
        .msg("Please enter your password (Enter with no input to return to previous screen): ")
        .get();
